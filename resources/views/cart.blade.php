  ...
  @csrf
  <input type="text" name="code" value="{{ old('code', $cart->promo_code) }}" class="flex-1 rounded-lg border-gray-300" placeholder="Enter code" {{ $cart->promo_code ? 'readonly' : '' }}>
  @if ($cart->promo_code)
   <button type="submit"
  ...
  @else
    <button type="submit" class="rounded-lg bg-slate-900 text-white px-3 py-2 hover:bg-slate-700">Apply</button>
  @endif
</form>
